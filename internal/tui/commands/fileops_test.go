...
func TestFileOps(t *testing.T) {
	...
	t.Run("Paste and Move Commands", func(t *testing.T) {
		...
		// Test pasteItems command execution (using same FS for src/dst)
		cmd := PasteItems(context.Background(), fs, fs, []string{src}, dest)
		...
		// Test MoveItems command execution (using same FS for src/dst)
		cmd = MoveItems(context.Background(), fs, fs, []string{filepath.Join(dest, "cmd_src.txt")}, tmpDir)
		batchMsg = cmd().(tea.BatchMsg)
		msg = batchMsg[1]()
		if _, ok := msg.(OperationFinishedMsg); !ok {
			t.Errorf("Expected OperationFinishedMsg on success, got %T (%v)", msg, msg)
	...
	t.Run("Delete Items Command Detailed", func(t *testing.T) {
		...
		// Success case
		cmd := DeleteItems(context.Background(), fs, []string{f1}, false)
		batchMsg := cmd().(tea.BatchMsg)
		msg := batchMsg[1]()
		if _, ok := msg.(OperationFinishedMsg); !ok {
			t.Errorf("Expected OperationFinishedMsg on success, got %T (%v)", msg, msg)
		...
		cmd = DeleteItems(context.Background(), mock, []string{"any-path"}, false)
		batchMsg = cmd().(tea.BatchMsg)
		msg = batchMsg[1]()
	...
	t.Run("Delete Items with Trash", func(t *testing.T) {
		...
		cmd := DeleteItems(context.Background(), fs, []string{f1}, true)
		batchMsg := cmd().(tea.BatchMsg)
		msg := batchMsg[1]()
	...
	t.Run("Paste and Move Error Cases", func(t *testing.T) {
		cmd := PasteItems(context.Background(), fs, fs, []string{"non-existent"}, tmpDir)
		batchMsg := cmd().(tea.BatchMsg)
		msg := batchMsg[1]()
		if _, ok := msg.(ErrorMsg); !ok {
		...
		cmd = MoveItems(context.Background(), fs, fs, []string{"non-existent"}, tmpDir)
		batchMsg = cmd().(tea.BatchMsg)
		msg = batchMsg[1]()
		if _, ok := msg.(ErrorMsg); !ok {
			t.Errorf("Expected ErrorMsg for non-existent source, got %T", msg)
	...
	t.Run("OverwriteItem Command", func(t *testing.T) {
		...
		cmd := OverwriteItem(context.Background(), fs, fs, src, dst, false)
		batchMsg := cmd().(tea.BatchMsg)
		msg := batchMsg[1]()
		if _, ok := msg.(OperationFinishedMsg); !ok {
	...
	t.Run("Paste and Move Conflict", func(t *testing.T) {
		...
		// Test paste conflict
		cmd := PasteItems(context.Background(), fs, fs, []string{src}, tmpDir)
		batchMsg := cmd().(tea.BatchMsg)
		msg := batchMsg[1]()
		...
		// Test move conflict
		cmd = MoveItems(context.Background(), fs, fs, []string{src}, tmpDir)
		batchMsg = cmd().(tea.BatchMsg)
		msg = batchMsg[1]()
		if cMsg, ok := msg.(ConflictMsg); !ok {
			t.Errorf("Expected ConflictMsg, got %T", msg)
